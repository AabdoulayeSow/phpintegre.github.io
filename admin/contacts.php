...
// Les chemins utilisent un seul '../' car le fichier est directement dans admin/
...
try {
    // Récupération de tous les messages de contact, du plus récent au plus ancien
    $stmt = $pdo->query("SELECT * FROM messages_contact ORDER BY id DESC");
    $contacts = $stmt->fetchAll();
} catch (PDOException $e) {
    error_log("Erreur Listing Contacts: " . $e->getMessage());
...
<head>
    ...
    <title>CORE_SYSTEM // INCOMING_CONTACTS</title>
    ...
    <main class="w-full max-w-6xl mx-auto px-8 py-12 flex flex-col gap-8">
        ...
        <header class="border-b border-[#00f719]/20 pb-6 flex justify-between items-end">
            <div>
                ...
                <h1 class="font-headline text-4xl font-black tracking-tighter uppercase italic text-[#F2F2F2]">MESSAGES DE CONTACT</h1>
            </div>
            <div class="font-mono text-xs text-[#A3A3A3]">
                TOTAL_MESSAGES: <span class="text-[#00f719] font-bold"><?= count($contacts) ?></span>
            </div>
        </header>

        <div class="w-full max-w-full border border-[#00f719]/20 bg-[#0A0A0A] overflow-x-auto">
            <table class="w-full text-left font-mono text-xs border-collapse min-w-[700px]">
                <thead>
                    <tr class="border-b border-[#00f719]/20 bg-black/50 text-[#A3A3A3] uppercase tracking-wider">
                        ...
                        <th class="p-4 w-44">Date Envoi</th>
                    ...
                    <?php if (empty($contacts)): ?>
                        <tr>
                            <td colspan="5" class="p-12 text-center text-[#A3A3A3] uppercase tracking-widest italic">
                                [// AUCUN MESSAGE DE CONTACT DANS LA BASE DE DONNÉES]
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($contacts as $msg): ?>
                            <tr class="hover:bg-white/[0.02] transition-colors <?= $msg['lu'] == 0 ? 'bg-[#00f719]/5' : '' ?>">
                                ...
                                <td class="p-4">
                                    <div class="text-[#F2F2F2] font-bold"><?= e($msg['nom']) ?></div>
                                    <div class="text-[#A3A3A3] text-[11px]"><?= e($msg['email']) ?></div>
                                </td>
                                <td class="p-4 text-[#e2e2e2]">
                                    <?= e($msg['sujet'] ?? 'Sans objet') ?>
                                </td>
                                <td class="p-4 text-[#A3A3A3]">
                                    <?= date('Y-m-d H:i:s', strtotime($msg['date_envoi'] ?? 'now')) ?>
                                </td>
                                <td class="p-4 text-right">
                                    <a href="lire_contact.php?id=<?= (int)$msg['id'] ?>" class="inline-block bg-white/5 border border-white/10 text-[#F2F2F2] hover:border-[#00f719] hover:text-[#00f719] px-3 py-1.5 transition-all uppercase tracking-wider text-[11px] font-bold">
                                        OPEN
                                    </a>
                                </td>
                            </tr>
