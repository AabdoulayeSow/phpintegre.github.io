...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nom) || empty($prenom) || empty($email) || empty($password)) {
        $erreur = "Tous les champs de sécurité sont obligatoires.";
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $erreur = "Format d'adresse e-mail invalide.";
    } else {
        try {
            // CORRECTION : Ciblage de la table 'administrateurs'
            $check = $pdo->prepare("SELECT id FROM administrateurs WHERE email = :email");
            $check->execute(['email' => $email]);
            
            if ($check->fetch()) {
                $erreur = "Cet identifiant e-mail est déjà alloué à un autre opérateur.";
            } else {
                // EXIGENCE SÉCURITÉ PROF : Hachage fort du mot de passe
                $passwordHash = password_hash($password, PASSWORD_DEFAULT);

                // CORRECTION : Insertion dans 'administrateurs'
                $stmt = $pdo->prepare("INSERT INTO administrateurs (nom, prenom, email, mot_de_passe) VALUES (:nom, :prenom, :email, :mdp)");
                $stmt->execute([
                    'nom'    => $nom,
                    'prenom' => $prenom,
                    'email'  => $email,
                ...
                $succes = "Nouvel opérateur injecté avec succès.";
                $nom = $prenom = $email = "";
            }
        } catch (PDOException $e) {
            error_log("Erreur Ajout Utilisateur: " . $e->getMessage());
            $erreur = "Erreur système lors de la création de la clé d'accès.";
        }
    }
}
?>
...
<head>
    ...
    <title>CORE_SYSTEM // REGISTER_OPERATOR</title>
...
<body class="font-sans antialiased">
    ...
    <main class="max-w-2xl mx-auto px-8 py-12 flex flex-col gap-8">
        ...
        <header class="border-b border-[#30363d] pb-6">
            ...
            <h1 class="font-headline text-3xl font-bold tracking-tight text-[#e2e2e2] mt-4 uppercase">ENREGISTRER UN OPÉRATEUR</h1>
        </header>
        ...
        <form action="ajouter.php" method="POST" class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-2">
                <label class="font-mono text-xs text-[#baccb0] uppercase">Clé d'accès (Mot de passe) :</label>
                <input type="password" name="password" required class="p-3 w-full" placeholder="••••••••••••" />
                <span class="font-mono text-[10px] text-[#baccb0]/50 italic">Sera haché cryptographiquement avant l'écriture BDD.</span>
            </div>
